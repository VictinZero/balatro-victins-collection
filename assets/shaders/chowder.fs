...
//
// Based on a shader by: stupxd
//

// Required for shader to not crash.
// chowder.y is the real time, and thus can be used to e.g. add an translation effect
extern PRECISION vec2 chowder;
...
extern PRECISION float card_scale;

// Normalized modulo
float norm_mod(float x, float y) {
    return mod(x, y)/y;
}
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    // We recompute the position of the pixel taken from the texture.
    ...
    float new_x = sprite_pos_x + norm_mod(screen_coords.x, card_scale*texture_details.z) * sprite_width;
    ...
    float new_y = sprite_pos_y + norm_mod(screen_coords.y, card_scale*texture_details.a) * sprite_height;

    // Take pixel color (rgba) from `texture` at `texture_coords`, equivalent of texture2D in GLSL
    vec4 tex = Texel(texture, vec2(new_x, new_y));
