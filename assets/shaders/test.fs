...
vec4 invert_colour(vec4 colour, bool invert_red, bool invert_green, bool invert_blue)
{
    return vec4(invert_red ? 1.0 - colour.r : colour.r,
                invert_green ? 1.0 - colour.g : colour.g,
                invert_blue ? 1.0 - colour.b : colour.b,
                colour.a);
}
...
vec4 increase_saturation(vec4 colour, float value)
{
    // vec4 original_colour = colour;
    vec4 hsl = HSL(colour);

    // Increase saturation
    hsl.y = min(hsl.y + value, 1.0);
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    number fac = 0.8 + 0.9*sin(11.*uv.x+4.32*uv.y + test.r*12. + cos(test.r*5.3 + uv.y*4.2 - uv.x*4.));
    number fac2 = 0.5 + 0.5*sin(8.*uv.x+2.32*uv.y + test.r*5. - cos(test.r*2.3 + uv.x*8.2));
    number fac3 = 0.5 + 0.5*sin(10.*uv.x+5.32*uv.y + test.r*6.111 + sin(test.r*5.3 + uv.y*3.2));
    number fac4 = 0.5 + 0.5*sin(3.*uv.x+2.32*uv.y + test.r*8.111 + sin(test.r*1.3 + uv.y*11.2));
    number fac5 = sin(0.9*16.*uv.x+5.32*uv.y + test.r*12. + cos(test.r*5.3 + uv.y*4.2 - uv.x*4.));
    ...
    // tex.r = tex.r*1.1 + (0.1*test.y);
    // // reduce the green channel and **SLIGHTLY** dependant on the rotation of the card
    // tex.g = tex.g*0.65 + (0.000001*test.x);
    // // greatly reduce the blue channel
    // tex.b = tex.b*0.2;

    // tex = increase_saturation(mix(tex, invert_colour(tex, true, true, true), 0.7), 1.0);
