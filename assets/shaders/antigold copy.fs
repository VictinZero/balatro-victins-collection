...
extern PRECISION vec2 antigold;
...
bool line(vec2 uv, float offset, float width) {
    ...
    offset = offset + 0.35 * sin(antigold.x + TWO_PI * lines_offset);
    width = width + 0.005 * sin(antigold.x);

    float min_y = -uv.x + offset;
    float max_y = -uv.x + offset + width;

    float t = antigold.y/7.;
...
void voronoise() {
    ...
    float m_dist = 0.1;  // minimum distance
    ...
    for (int y= -1; y <= 1; y++) {
        for (int x= -1; x <= 1; x++) {
			...
			// Animate the point
            point = 0.5 + 0.5*sin(0.*u_time + 6.2831*point);
            vec2 point2 = 0.5 + 0.5*sin(0.*u_time + 1. + 6.2831*point);
            point = point2;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    pixel.rgb = mix(original_colour.rgb, pixel.rgb, 1. - V);
