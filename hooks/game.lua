...
function Game:init_game_object()
    ...
    init_game_object_val.VictinsCollection = {
        ...
        mask_blinds = {'bl_goad', 'bl_plant', 'bl_wall'},
        malachite_blinds = {'bl_final_bell', 'bl_final_leaf', 'bl_final_vessel'},
        ...
        zodiac_dict = {
            ...
            ['Pair'] = 'taurus',
            ['Two Pair'] = 'gemini',
            ['Three of a Kind'] = 'cancer',
            ['Straight'] = 'leo',
            ['Flush'] = 'virgo',
            ['Full House'] = 'libra',
            ['Four of a Kind'] = 'scorpio',
            ['Straight Flush'] = 'sagittarius',
            ['Five of a Kind'] = 'capricorn',
            ['Flush House'] = 'aquarius',
            ['Five of a Flush'] = 'pisces'
        }
    }
...
function Game:update(dt)
    ...
    local ordered_suit_colors = {suit_colors['Spades'], suit_colors['Hearts'], suit_colors['Clubs'],
    ...
    local ordered_poker_hands = {'High Card', 'Pair', 'Two Pair', 'Three of a Kind', 'Straight', 'Flush', 'Full House',
                                 'Four of a Kind', 'Straight Flush', 'Five of a Kind', 'Flush House', 'Five of a Flush'}
    local hand_colors = {}
...
function Game.update_new_round(self, dt)
    ...
    if not G.STATE_COMPLETE then
        if (G.GAME.blind.config.blind.key == "bl_vic_final_loop" or
            (G.GAME.blind.config.blind.key == "bl_vic_loop" and not G.GAME.blind.vic_looped)) and
            (not G.GAME.blind.disabled) then

            local original_blind = G.GAME.blind.vic_original_blind or G.GAME.blind.config.blind.key
            G.GAME.blind.vic_looped = (G.GAME.blind.config.blind.key == "bl_vic_loop")
            ...
            G.GAME.current_round.dollars_to_be_earned = G.GAME.blind.dollars > 0 and
                                                            (string.rep(localize('$'), G.GAME.blind.dollars) .. '') or
                                                            ('')
            ...
            G.E_MANAGER:add_event(Event({
                ...
                blocking = false,
                func = function()
                    if G.GAME.current_round.hands_left <= 0 then -- to_big(G.GAME.chips) >= to_big(G.GAME.blind.chips) then
        ...
        elseif (G.GAME.blind.config.blind.key == "bl_vic_loop" and G.GAME.blind.vic_looped) then
            G.GAME.blind.vic_looped = false
        elseif (G.GAME.blind.vic_patriarch_blinds) and (#G.GAME.blind.vic_patriarch_blinds > 0) then

            local original_blind = G.GAME.blind.vic_original_blind and G.GAME.blind.vic_original_blind or
