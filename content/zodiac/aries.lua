...
local loc_vars = function(self, info_queue, card)
    local level = G.GAME and G.GAME.VictinsCollection.zodiac.aries or card.ability.extra.level
...
return {
    ...
    set_ability = function(self, card, initial, delay_sprites)
        if not misc.is_in_your_collection and G.ca_vic_zodiac then
            for _, v in ipairs(G.ca_vic_zodiac.cards) do
                if v.config.center.key == 'j_vic_aries' then
    ...
    use = function(self, card, area, copier)
        for _, v in ipairs(G.ca_vic_zodiac.cards) do
            if v.config.center.key == 'j_vic_aries' then
                G.GAME.VictinsCollection.zodiac.aries = G.GAME.VictinsCollection.zodiac.aries + 1
                v.ability.extra.level = G.GAME.VictinsCollection.zodiac.aries
                v:juice_up()
            end
        end
    end,
