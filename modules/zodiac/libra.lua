...
return {
    ...
    set_ability = function(self, card, initial, delay_sprites)
        if not misc.is_in_your_collection and G.ca_vic_zodiac then
            for _, v in ipairs(G.ca_vic_zodiac.cards) do
                if v.config.center.key == 'j_vic_libra' then
    ...
    use = function(self, card, area, copier)
        for _, v in ipairs(G.ca_vic_zodiac.cards) do
            if v.config.center.key == 'j_vic_libra' then
                G.GAME.VictinsCollection.zodiac.libra = G.GAME.VictinsCollection.zodiac.libra + 1
                v.ability.extra.level = G.GAME.VictinsCollection.zodiac.libra
                v:juice_up()
            end
        end
    end,
